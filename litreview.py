...
class LitreviewShell(cmd2.Cmd):
    ...
    def get_notes_at_current_level(self, target_obj):
        notetypes = self.db.get_notetypes_by_obj(target_obj)
        notes = []
        for notetype in notetypes:
            for note in self.db.get_notes_by_obj(target_obj, notetype):
                notes.append(note)
        self.note_index_at_current_level = 0
        return notes

    def show_note_info(self, current_note):
        self.print_indented("Notetype: {0}".format(current_note.notetype))
        self.print_indented("Body: {0}".format(current_note.body))
        self.print_indented("Attached notetypes:")
